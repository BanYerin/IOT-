/*
-작성자: 2017038023 반예린(DB, UI), 2015023025 배나영(UI)
-해당 소스파일 정보: 사용자 정보 등록 및 수정에 대한 기능.
                    사용자 주소, 신고 연락처, 아두이노 서버 IP주소를 입력한 뒤 등록 버튼을 클릭하면 DB에 존재하는 기존 정보를 삭제하고 새로 입력한 정보를 삽입함.
...
public class SettingActivity extends AppCompatActivity {
    ...
    public void printUserInfo(){
        //사용자 주소, 관리자 전화번호, 아두이노 서버 IP주소 정보를 EditText형으로 받아옴
        EditText userAddrEditText=findViewById(R.id.userAddress);
        EditText adminNumEditText=findViewById(R.id.adminPhoneNumber);
        EditText camIpEditText=findViewById(R.id.camIp);

        //사용자 주소, 관리자 전화번호, 아두이노 서버 IP주소 정보를 TextView형으로 받아옴
    ...
    public void OnclickRegister(View v){
        ...
        //사용자로 부터 입력받은 사용자 주소, 관리자 연락처, 아두이노 서버 IP주소를 UserInfo 테이블에 삽입
        db.execSQL("insert into UserInfo values(null, '"+userAddrString+"', '"+adminPhoneNumString+"', '"+ipString+"');");

        printUserInfo(); //변경된 사용자 정보를 화면에 출력

        // 받아온 사용자 주소, 관리자 전화번호, 아두이노 서버 IP주소 정보를 화면에 띄워줌
        Toast.makeText(getApplicationContext(),"<등록 완료>"+"\n사용자주소 : "+userAddrString+"\n관리자 연락처 : "+adminPhoneNumString+ "\n아두이노 서버 IP주소 : "+ipString,Toast.LENGTH_LONG).show();
