...
//            db.execSQL("insert into DataSensing values(null, '2020-01-05 15:36:12', 126.65, 81.29);");
    ...
    public void printFireRecord(){
        ...
        db=mHelper.getReadableDatabase(); //읽기 가능한 SQLite DB 인스턴스 생성
        cursor=db.rawQuery("select time, temper, gas from DataSensing order by time desc", null);

        //결과가 존재하는 동안 레코드를 읽어와 각 필드에 대입하고 각 레코드에 대한 화재감지 기록 출력
        while(cursor.moveToNext()){
            //결과가 존재하는 동안 레코드를 읽어와 각 필드에 대입
            detectTime=cursor.getString(0);
            temp=cursor.getFloat(1);
            ...
            DinamicView.setText(detectTime+"             "+temp+"        "+gas+"\n");
    ...
    //새로고침 메소드: 화면의 화재감지 기록 출력물을 업데이트함
